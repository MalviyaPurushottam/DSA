...
static avl_node* avl_r_left_left(avl_node* node);
static avl_node* avl_r_left_right(avl_node* node);
static avl_node* avl_r_right_left(avl_node* node);
static avl_node* avl_r_right_right(avl_node* node);

static int check_balance(avl_node* node);
static avl_node* balance_subtree(avl_node* node);
...
void AVLTree_insertNode(avl_tree* Tree, void* new_node, int (*compare)(void*a, void*b)){
    ...
    while (temp!= NULL) {
        ...
        if(check_balance(temp) == BALANCED){
...
static avl_node* avl_r_left_left(avl_node* node){
    ...
    */
    ...
    // updating the height of the x z
    x->height = y->height -1;
    z->height = y->height -1;
...
static avl_node* avl_r_left_right(avl_node* node){
    ...
    */
    ...
    // taking the height of x as it remains same
    x->height = z->height;
    ...
    // updating the height of the y z
    y->height = x->height - 1;
    z->height = x->height - 1;
